<div>
    ...
    @if(auth()->user()->role == 'pimpinan')
        ...
        <div class="bg-gray-50 border-l-4 border-gray-400 p-6 rounded-r-xl shadow-sm mb-8">
            <div class="flex items-start gap-4">
                <div class="p-2 bg-gray-200 text-gray-600 rounded-full">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>
                <div>
                    <h3 class="text-lg font-bold text-gray-800">Mode Monitoring (Draft)</h3>
                    <p class="text-gray-600 text-sm mt-1">
                        Dokumen ini masih dalam status <strong>Draft</strong>. Pegawai pengampu ({{ $pegawai->nama ?? 'Pegawai' }}) belum melakukan "Simpan & Publikasi".
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 text-sm mt-1">Dokumen ini telah diajukan. Silakan periksa dan tentukan keputusan.</p>
                    </div>
                </div>
                <div class="flex gap-3 w-full md:w-auto">
                    <button wire:click="openModalVerifikasi('ditolak')" class="px-5 py-2.5 bg-white border border-red-300 text-red-600 font-bold rounded-lg hover:bg-red-50 transition-colors shadow-sm">
                        Tolak / Revisi
                    </button>
                    <button wire:click="openModalVerifikasi('disetujui')" class="px-5 py-2.5 bg-gray-800 text-white font-bold rounded-lg hover:bg-gray-900 transition-colors shadow-md">
                        Setujui (Final)
                    </button>
                </div>
            </div>
        ...
        <div class="bg-green-50 border-l-4 border-green-500 p-6 rounded-r-xl shadow-sm mb-8">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-green-700 text-sm mt-1">Diverifikasi pada: <strong>{{ \Carbon\Carbon::parse($pk->tanggal_verifikasi)->translatedFormat('d F Y H:i') }}</strong></p>
                </div>
            </div>
        </div>
        @endif
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                ...
                @elseif($pk->status_verifikasi == 'pending')
                    <span class="w-fit bg-yellow-100 text-yellow-700 text-xs font-bold px-2.5 py-1 rounded border border-yellow-300 uppercase">MENUNGGU VERIFIKASI</span>
                @elseif($pk->status_verifikasi == 'disetujui')
                    <span class="w-fit bg-green-100 text-green-700 text-xs font-bold px-2.5 py-1 rounded border border-green-300 uppercase">FINAL / DISETUJUI</span>
            ...
            <div class="flex gap-2 w-full md:w-auto">
                ...
                <a href="{{ route('perjanjian.kinerja.print', $pk->id) }}" target="_blank" class="flex-1 md:flex-none inline-flex justify-center items-center px-4 py-2 bg-yellow-400 hover:bg-yellow-500 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                    Preview Cetak
                </a>
                
                @if(in_array($pk->status_verifikasi, ['draft', 'ditolak']) && in_array(auth()->user()->role, ['pegawai', 'admin']))
        ...
        <div class="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
            <p class="text-xs font-bold text-red-600 uppercase mb-1">Catatan Revisi dari Pimpinan:</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-3 relative overflow-hidden">
            ...
            <div><span class="text-xs text-gray-500 font-medium uppercase">PD:</span><span class="text-sm font-bold text-gray-900 ml-1">DINAS KESEHATAN</span></div>
            <div><span class="text-xs text-gray-500 font-medium uppercase">Jabatan PK:</span><span class="text-sm font-bold text-gray-900 ml-1">{{ $jabatan->nama }}</span></div>
            <div><span class="text-xs text-gray-500 font-medium uppercase">Tahun:</span><span class="text-sm font-bold text-gray-900 ml-1">{{ $pk->tahun }}</span></div>
            <div><span class="text-xs text-gray-500 font-medium uppercase">Keterangan:</span><span class="text-sm font-bold text-gray-900 ml-1">{{ $pk->keterangan }}</span></div>
        </div>
        
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 relative overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-1 bg-gray-800"></div>
            <div class="flex justify-between items-start">
                <h4 class="font-bold text-gray-900 mb-3">Pihak 1</h4>
                <span class="text-[10px] bg-gray-100 text-gray-700 px-2 py-0.5 rounded font-bold uppercase">Pembuat</span>
            </div>
            @if($pegawai)
                <div class="flex items-center gap-3">
                    <div class="h-10 w-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 border border-gray-200">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                    </div>
                    <div>
                        <div class="text-sm font-bold text-gray-900 uppercase leading-tight">{{ $pegawai->nama }}</div>
                        <div class="text-xs text-gray-500 mt-0.5">NIP: {{ $pegawai->nip }}</div>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 relative overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-1 bg-gray-800"></div>
            <div class="flex justify-between items-start">
                <h4 class="font-bold text-gray-900 mb-3">Pihak 2 (Atasan)</h4>
                <span class="text-[10px] bg-gray-100 text-gray-700 px-2 py-0.5 rounded font-bold uppercase">Verifikator</span>
            </div>
            <div class="flex items-center gap-3">
                <div class="h-10 w-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 border border-gray-200">
                ...
                <div>
                    @if($is_kepala_dinas)
                        <div class="text-sm font-bold text-gray-900 uppercase leading-tight">{{ $gubernur_nama }}</div>
                        <div class="text-xs text-gray-500 mt-0.5">{{ $gubernur_jabatan }}</div>
                    @elseif($atasan_pegawai)
                        <div class="text-sm font-bold text-gray-900 uppercase leading-tight">{{ $atasan_pegawai->nama }}</div>
                        <div class="text-xs text-gray-500 mt-0.5">{{ $atasan_jabatan->nama ?? 'Atasan' }}</div>
    ...
    @if(in_array(auth()->user()->role, ['pegawai', 'admin']))
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 mb-6 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
            <div class="flex-1">
                <h4 class="font-bold text-gray-800">Aksi</h4>
            ...
            <div class="flex gap-3 w-full sm:w-auto">
                ...
                <button wire:click="ajukan" wire:confirm="Kirim data ke Pimpinan?" class="flex-1 sm:flex-none bg-pink-500 hover:bg-pink-600 text-white text-sm font-bold px-4 py-2 rounded-lg shadow-md flex items-center justify-center transition-colors">
        ...
        <div class="mb-6 flex items-center text-gray-800 bg-gray-50 px-4 py-3 rounded-lg border border-gray-200 shadow-sm">
            <svg class="w-5 h-5 mr-3 animate-spin text-gray-500" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
            <div class="flex-1">
                <span class="font-bold">Sedang Diverifikasi Pimpinan.</span> Anda tidak dapat mengedit data saat ini.
            </div>
            <button wire:click="batalkan" wire:confirm="Batalkan pengajuan?" class="ml-4 text-xs font-bold text-gray-600 underline hover:text-gray-800">Batalkan Pengajuan</button>
        </div>
        @endif
    @endif
        ...
        <div class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow" x-data="{ expanded: true }">
            <div class="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center cursor-pointer" @click="expanded = !expanded">
                <h4 class="text-sm font-bold text-gray-800 uppercase tracking-wide flex items-center">
                    <span class="bg-gray-200 text-gray-800 text-[10px] px-2 py-0.5 rounded mr-3">#{{ $index + 1 }}</span>
                    Kinerja Utama
                </h4>
            ...
            <div x-show="expanded" class="p-6">
                <div class="flex justify-between items-start gap-4 mb-6">
                    <div class="flex-1">
                        <p class="text-base font-bold text-gray-900 leading-relaxed">{{ $sasaran->sasaran }}</p>
                    </div>
                    @if(in_array($pk->status_verifikasi, ['draft', 'ditolak']) && in_array(auth()->user()->role, ['pegawai', 'admin']))
                ...
                <div class="overflow-x-auto border border-gray-100 rounded-lg">
                    <table class="w-full text-left text-sm">
                        <thead class="bg-gray-50 text-gray-800 text-xs uppercase font-bold border-b border-gray-200">
                            <tr>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr wire:key="ind-{{ $ind->id }}" class="hover:bg-gray-50">
                                @if($editingIndikatorId === $ind->id && in_array(auth()->user()->role, ['pegawai', 'admin']))
                                    <td class="px-4 py-3 text-gray-800 font-medium">{{ $ind->nama_indikator }}</td>
                                    <td class="px-4 py-3 text-center">{{ $ind->satuan }}</td>
                                    <td class="px-4 py-3 text-center">
                                        <input type="text" wire:model="editTargetValue" class="w-24 border border-gray-300 rounded px-2 py-1 text-center text-sm focus:ring-2 focus:ring-gray-500 outline-none">
                                    </td>
                                    <td class="px-4 py-3 text-center">{{ $ind->arah }}</td>
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex justify-center gap-1">
                                            <button wire:click="saveEdit" class="text-white bg-green-600 hover:bg-green-700 px-2 py-1 rounded text-xs font-bold">Simpan</button>
                                ...
                                @else
                                    <td class="px-4 py-3 text-gray-800 font-medium">{{ $ind->nama_indikator }}</td>
                                    <td class="px-4 py-3 text-center text-gray-600 bg-gray-50">{{ $ind->satuan }}</td>
                                    <td class="px-4 py-3 text-center font-bold text-gray-900 bg-gray-100 border-l border-gray-200">
                                        @php $col = 'target_'.$pk->tahun; echo $ind->$col ?? $ind->target; @endphp
                                    </td>
                                    <td class="px-4 py-3 text-center text-gray-600">{{ $ind->arah }}</td>
                                    <td class="px-4 py-3 text-center">
                                        @if(in_array($pk->status_verifikasi, ['draft', 'ditolak']) && in_array(auth()->user()->role, ['pegawai', 'admin']))
                                        <div class="flex justify-center gap-2">
                                            <button wire:click="startEdit({{ $ind->id }})" class="text-gray-600 hover:text-gray-800 text-xs font-bold flex items-center">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-gray-800">
                            @if($ang->subKegiatan)
                                <div class="font-bold text-gray-600 text-xs">{{ $ang->subKegiatan->kode }}</div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm" x-data>
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden transform transition-all animate-fade-in-down">
            ...
            <div class="p-6 space-y-4">
                <div class="bg-gray-50 p-3 rounded-lg flex gap-3 items-start">
                    <svg class="w-5 h-5 text-gray-500 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                ...
                <div>
                    ...
                    <textarea wire:model="catatan_pimpinan" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-gray-500 focus:border-gray-500 placeholder-gray-400" placeholder="Tulis catatan atau arahan untuk pegawai..."></textarea>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <select wire:model.live="sub_kegiatan_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-gray-500">
                ...
                <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    <p class="text-xs font-bold text-gray-500 uppercase mb-1">Output (Sasaran Kinerja)</p>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Sub Kegiatan</label>
                    <select wire:model="anggaran_sub_kegiatan_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-gray-500">
                ...
                <div>
                    ...
                    <input type="number" wire:model="anggaran_nilai" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-gray-500" placeholder="0">
                </div>
            </div>
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                <button wire:click="closeModal" class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">Batal</button>
