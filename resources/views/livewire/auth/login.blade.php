<div class="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#0a0f1d] font-sans">
    ...
    <style>
        ...
        /* Styling Scrollbar untuk Dropdown agar rapi */
        select option {
            background-color: #1e293b; /* Warna gelap untuk opsi */
            color: white;
            padding: 10px;
        }
    </style>
        ...
        <div class="bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[2rem] shadow-2xl overflow-hidden">
            ...
            <div class="p-8 md:p-9">
                ...
                <form wire:submit="login" class="space-y-4">
                    ...
                    {{-- INPUT DROPDOWN JABATAN --}}
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-bold text-blue-400/80 uppercase tracking-[0.15em] ml-1">Pilih Jabatan (Username)</label>
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 z-10">
                            ...
                            <select wire:model="username" 
                                    class="w-full pl-10 pr-10 py-3 bg-white/[0.03] border border-white/10 rounded-xl text-white appearance-none focus:outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/10 transition-all cursor-pointer text-sm">
                                
                                <option value="" class="bg-[#1e293b] text-slate-400">-- Pilih Jabatan Anda --</option>
                                
                                {{-- 1. OPSI ADMIN --}}
                                <option value="admin" class="bg-blue-900 text-yellow-400 font-bold tracking-wide">
                                    ★ Administrator Sistem
                                </option>

                                {{-- 2. OPSI PIMPINAN (BARU) --}}
                                <option value="pimpinan" class="bg-green-900 text-green-400 font-bold tracking-wide">
                                    ★ Kepala Dinas Kesehatan
                                </option>

                                <option disabled class="bg-[#1e293b] text-slate-500">-----------------------------------</option>
                                
                                {{-- 3. LOOPING JABATAN DARI DATABASE --}}
                                @foreach($jabatans as $jabatan)
                                    <option value="{{ $jabatan->nama }}" class="bg-[#1e293b] text-white">
                                        {{ $jabatan->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="space-y-1.5" x-data="{ show: false }">
                        <label class="text-[10px] font-bold text-blue-400/80 uppercase tracking-[0.15em] ml-1">Password (NIP)</label>
                        <div class="relative group">
