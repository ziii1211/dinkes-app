<div>
    ...
    <div class="space-y-6">
        ...
        {{-- BAGIAN 2: TABEL CROSSCUTTING (Judul & Tombol Diperbaiki) --}}
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-white">
        ...
        {{-- BAGIAN 3: VISUALISASI POHON KINERJA (ZOOMABLE & TREE STRUCTURE) --}}
                    ...
                    <div class="flex flex-col gap-16 items-center">
                        ...
                        <div class="flex flex-col items-center">
                            ...
                            @if($root->children->count() > 0)
                                {{-- 1. Garis Vertikal dari Induk ke Bawah --}}
                                <div class="h-12 w-px bg-gray-400"></div>

                                {{-- 2. Container Horizontal untuk Anak-anak --}}
                                <div class="flex gap-16 items-start relative">
                                    ...
                                    @if($root->children->count() > 1)
                                        ...
                                        {{-- Note: 664px = 600px (lebar kotak) + 64px (gap/16*4) --}}
                                    @endif

                                    @foreach($root->children as $child)
                                    <div class="flex flex-col items-center relative">
                                        
                                        {{-- 3. Garis Vertikal Kecil ke Setiap Anak --}}
                                        <div class="h-8 w-px bg-gray-400 -mt-8 mb-0"></div> 
                                        
                                        {{-- === KOTAK ANAK (SAMA PERSIS DENGAN INDUK) === --}}
                                        <div class="bg-white border border-gray-300 shadow-lg max-w-2xl w-[600px] rounded-sm overflow-hidden relative z-10">
                                            <div class="bg-white p-2 border-b border-gray-200 text-center">
                                                <h2 class="font-bold text-gray-800 text-sm uppercase tracking-wide">DINAS KESEHATAN</h2> 
                                            </div>
                                        ...
                                        {{-- === LEVEL 3: CUCU (GRANDCHILDREN) - (BARU DITAMBAHKAN) === --}}
                                        @if($child->children->count() > 0)
                                            <div class="h-12 w-px bg-gray-400"></div> {{-- Garis turun dari Anak ke Cucu --}}
                                            
                                            <div class="flex gap-16 items-start relative">
                                                
                                                {{-- Garis Horizontal Penghubung Cucu --}}
                                                ...
                                                <div class="flex flex-col items-center relative">
                                                    ...
                                                    {{-- Garis Vertikal Kecil ke Setiap Cucu --}}
                                                    <div class="h-8 w-px bg-gray-400 -mt-8 mb-0"></div>
                                                    
                                                    {{-- BOX CUCU (SAMA PERSIS DENGAN INDUK & ANAK) --}}
                                                    <div class="bg-white border border-gray-300 shadow-lg max-w-2xl w-[600px] rounded-sm overflow-hidden relative z-10">
                                                        <div class="bg-white p-2 border-b border-gray-200 text-center">
                                                            <h2 class="font-bold text-gray-800 text-sm uppercase tracking-wide">DINAS KESEHATAN</h2> 
                                                        </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center"><h3 class="text-lg font-bold text-gray-800">Tambah Crosscutting</h3><button wire:click="closeModal" class="text-gray-400 hover:text-gray-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div>
            <div class="p-6 space-y-4">
                @if($active_pohon_id)<div class="bg-blue-50 p-3 rounded text-xs text-blue-700 mb-2 border border-blue-100"><strong>Kinerja Sumber:</strong> <span class="italic">Terpilih otomatis.</span></div>@endif
                <div><label class="block text-sm font-semibold text-gray-700 mb-2">Pilih Unit Kerja (SKPD)</label><select wire:model="cross_skpd_id" class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm"><option value="">-- Pilih Unit Kerja --</option>@foreach($skpds as $s) <option value="{{ $s->id }}">{{ $s->nama_skpd }}</option> @endforeach</select>@error('cross_skpd_id') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror</div>
                <div><label class="block text-sm font-semibold text-gray-700 mb-2">Pilih Kinerja (Referensi)</label><select wire:model="cross_tujuan_id" class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm"><option value="">-- Pilih Kinerja --</option>@foreach($all_pohons as $p) <option value="{{ $p->id }}">{{ $p->nama_pohon }}</option> @endforeach</select>@error('cross_tujuan_id') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror</div>
            </div>
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3"><button wire:click="closeModal" class="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg">Batal</button><button wire:click="storeCrosscutting" class="px-5 py-2.5 bg-blue-600 text-white rounded-lg">Simpan</button></div>
        </div>
    </div>
    @endif
</div>