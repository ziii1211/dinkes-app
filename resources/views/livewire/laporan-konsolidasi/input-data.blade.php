<div class="space-y-6"
    x-data="{ 
        // SINKRONISASI TOTAL DARI BACKEND LIVEWIRE (MENGCAKUP SEMUA HALAMAN)
        totalAnggaran: @entangle('totalAnggaran'), 
        totalRealisasi: @entangle('totalRealisasi'),
        
        // Menggunakan getter agar otomatis terhitung saat nilai total berubah
        get totalPersentase() {
            let ang = parseFloat(this.totalAnggaran) || 0;
            let real = parseFloat(this.totalRealisasi) || 0;
            
            // RUMUS PERSENTASE (Realisasi / Anggaran x 100)
            if (ang > 0) {
                let persen = (real / ang) * 100;
