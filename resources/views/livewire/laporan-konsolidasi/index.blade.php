                        ...
                        <tr class="hover:bg-blue-50/30 transition-colors group">
                            ...
                            <td class="px-6 py-5">
                                <div class="flex items-center justify-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                    ...
                                    {{-- TOMBOL UTAMA: ISI DATA / VERIFIKASI DATA --}}
                                    <a href="{{ route('laporan-konsolidasi.input', $laporan->id) }}"
                                        class="flex items-center gap-2 px-3 py-2 
                                        {{-- Warna berbeda untuk Verifikator --}}
                                        {{ auth()->user()->role == 'verifikator' ? 'bg-indigo-50 text-indigo-700 hover:bg-indigo-600 hover:text-white border-indigo-200 hover:border-indigo-600' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-600 hover:text-white border-emerald-200 hover:border-emerald-600' }}
                                        rounded-lg border transition-all duration-200 text-xs font-bold uppercase tracking-wider shadow-sm group/btn">
                                        
                                        @if(auth()->user()->role == 'verifikator')
                                            {{-- Icon Verifikasi (Centang) --}}
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                            </svg>
                                            Verifikasi Data
                                        @else
                                            {{-- Icon Isi Data (Pensil/Edit) --}}
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center justify-center animate-fade-in-down">
                                    ...
                                    {{-- TOMBOL CREATE EMPTY STATE: HANYA ADMIN --}}
                                    @if(auth()->user()->role == 'admin')
                    ...
                    <tbody wire:loading class="w-full bg-white divide-y divide-gray-100">
                        @for ($i = 0; $i < 5; $i++)
                            ...
                            <td class="px-6 py-5 text-center">
                                <div class="h-4 bg-gray-200 rounded w-4 mx-auto"></div>
                            </td>
                            <td class="px-6 py-5">
                            ...
                            <td class="px-6 py-5 text-center">
                                <div class="h-8 bg-gray-200 rounded-md w-full"></div>
                            </td>
                            </tr>
                            @endfor
                    </tbody>
